...
fn main() {
    ...
    // Create separate channels for each child thread
    let (tx_a, rx_a) = channel::unbounded();
    let (tx_b, rx_b) = channel::unbounded();

    let handle_ca = thread::spawn(move || {
        for msg in rx_a {
            println!("Child thread A: Received {}", msg);
        }
    });

    let handle_cb = thread::spawn(move || {
        for msg in rx_b {
            println!("Child thread B: Received {}", msg);
        }
    });

    // From the main thread loop through several values and print each out and then send it to the channel
    for value in vec!["one", "two", "three", "four", "five"] {
        println!("Main thread: Sending {}", value);
        tx_a.send(value).unwrap();
        tx_b.send(value).unwrap();
    }

    // Close the sending side in the main thread
    drop(tx_a);
    drop(tx_b);

    // Wait for child threads to finish
    handle_ca.join().unwrap();
    handle_cb.join().unwrap();
